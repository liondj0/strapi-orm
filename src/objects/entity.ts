...
export type FetchEntityOptions = { relations?: string[]; query: Query };

export abstract class Entity extends StrapiObject {
    async fetch(url: string, query?: FetchEntityOptions) {
        const queryWithDefaultRelations: Query = { ...query?.query, ...{ relations: query?.relations ?? getFlatRelationsForObject(this.getInstance()) } };
