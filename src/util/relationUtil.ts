...
const getRelations = <T extends StrapiObject>(target: any): Relation[] => {
    const objectFields: Relation[] = Reflect.getMetadata(CONSTANTS.FIELDS_KEY, target)
        .filter((field: Field<T>) => [FieldType.RELATION, FieldType.COMPONENT].includes(field.type))
        .map((field: Field<T>) => ({ field, relations: [] }));
